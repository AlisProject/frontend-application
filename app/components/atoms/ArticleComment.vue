    ...
    <div>
      <div class="article-comment">
        <nuxt-link :to="`/users/${comment.userInfo.user_id}`" class="commented-user">
          <img v-if="hasUserIcon" class="icon" :src="comment.userInfo.icon_image_url" alt="Profile icon">
        ...
        <div v-if="showDeleteAction" class="action-delete" @click="toggleDeleteCommentPopup">
          <img class="icon" src="~assets/images/pc/article/a_icon_menu.png" alt="Delete comment">
  ...
  methods: {
    ...
    async deleteComment(event) {
      try {
        ...
        if (error.response.data.message === 'Record Not Found') {
          this.sendNotification({
            text: 'コメントはすでに削除されています。削除が反映されるまでしばらくお待ち下さい',
            type: 'warning'
          })
        }
      }
